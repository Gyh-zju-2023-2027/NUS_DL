...
def combine_expert_suggestions(expert_suggestions: List[RoundExpertSuggestions]) -> RoundExpertSuggestions:
    combined_expert_suggestions: RoundExpertSuggestions = RoundExpertSuggestions(
        ...
        suggestion_keys=expert_suggestions[0].suggestion_keys+expert_suggestions[1].suggestion_keys
    )
...
def load_and_extract_sugg_from_object(sugg_object: List[Dict]) -> Tuple[
    List[ParsedSuggestion], List[AllowedSuggestionKey]]:
    sugg_list = []
...
def load_and_extract_sugg_from_object_stroke(sugg_object: List[Dict]) -> Tuple[
    List[StrokeExpertSuggestion], List[AllowedSuggestionKey]]:
                ...
                if os.path.isdir(tech_dir):
                    for round_id in ['round_00', 'round_01', 'round_02']: 
                        ...
                        if os.path.isdir(round_dir):
                            ...
                            # region load video data
                            video_remote_dir = os.path.join(round_dir, "video", "remote")
                            video_path = ""
                            has_video_error = False
                            if os.path.isdir(video_remote_dir):
                                video_files = [
                                    f for f in os.listdir(video_remote_dir)
                                    if os.path.isfile(os.path.join(video_remote_dir, f))
                                ]
                                if video_files:
                                    video_path = os.path.join(video_remote_dir, video_files[0])
                                else:
                                    has_video_error = True
                            else:
                                has_video_error = True
                            if has_video_error:
                                video_right_dir = os.path.join(round_dir, "video", "right") # only a few lack the video from the remote camera, so we use the camera in the right
                                if os.path.isdir(video_right_dir):
                                    video_files = [
                                        f for f in os.listdir(video_right_dir)
                                        if os.path.isfile(os.path.join(video_right_dir, f))
                                    ]
                                    if video_files:
                                        video_path = os.path.join(video_right_dir, video_files[0])

                            # endregion
                            # load sensor data
                            sensor_dir = os.path.join(round_dir, "sensor/aligned.json")
                            try:
                            ...
                            for stroke in sensor_data_json:
                                ...
                                acc_peak_exp_sqrt_full = np.sqrt(
                                    full_transposed_data_np[0, :] ** 2 + full_transposed_data_np[1,
                                                                         :] ** 2 + full_transposed_data_np[2, :] ** 2)
                                agl_spd_peak_exp_sqrt_full = np.sqrt(
                                    full_transposed_data_np[3, :] ** 2 + full_transposed_data_np[4,
                                                                         :] ** 2 + full_transposed_data_np[5, :] ** 2)
                                agl_y_peak_full = full_transposed_data_np[7, :]
                                transposed_values_aggregated = [acc_peak_exp_sqrt_full, agl_y_peak_full,
                                                                agl_spd_peak_exp_sqrt_full]
                                transposed_values = full_transposed_data
                                extracted_sensor_data.append(
                                    transposed_values_aggregated if FLAG_SENSECOACH else transposed_values)

                            pose_dir = os.path.join(round_dir, "pose/aligned_downsampled.json")
                            try:
                            ...
                            suggestion_dir = os.path.join(round_dir, "coach_suggestions")
                            ...
                            if len(extracted_sensor_data) != len(extracted_pose_data):
                                print("len(extracted_sensor_data) != len(extracted_pose_data)",
                                      len(extracted_sensor_data), len(extracted_pose_data), round_meta_info)
...
def preprocess_data_as_data_loader(round_data_list_: List[RoundDataIncludesPoseSensor], batch_size: int, fps: int):
    ...
    def process_data(data_list: List[RoundDataIncludesPoseSensor]):
        ...
        return list(zip(sensor_tensors, pose_tensors, mask_tensors)), list(
            zip(round_meta_info_list, sugg_key_type_set_list, label_tensor))
