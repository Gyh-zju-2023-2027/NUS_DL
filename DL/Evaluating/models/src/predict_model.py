...
from .data_encoder import PoseEncoder
...
class FusionModel(nn.Module):
    def __init__(self, pose_input_size, USE_LSTM_LAYER, USE_LEARNABLE_MAPPING, USE_SENSOR_PROCESS, hidden_size=64):
        super().__init__()
    ...
    def _build_mapping_matrix(self, mapping, total_data_keys: int):
        suggkeyid_datakey_id_mapping = {}
        for sugg_key, list_data_key in mapping.items():
            # print(list_data_key)
            # Filter out keys not in all_data_keys
            valid_data_keys = []
            for key in list_data_key:
                try:
                    valid_data_keys.append(all_data_keys.index(key))
                except ValueError:
                    print(f"Warning: data key '{key}' not in current data key list, ignored")
            if valid_data_keys:  # Only add mapping if there are valid keys
                suggkeyid_datakey_id_mapping[allowed_sugg_keys.index(sugg_key)] = valid_data_keys
    ...
    def forward(self, pose_x, stroke_mask):
        pose_feat = self.pose_encoder(pose_x)  # [B, S, poseK, H]
        all_feat = pose_feat  # [B, S, poseK, H]
